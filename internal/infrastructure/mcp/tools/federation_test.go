...
func TestFederationTools_ExecuteAndExecuteTool_ZeroValueInternalHubRejectsMutations(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			args: map[string]interface{}{
				"swarmId":   "zero-value-hub-swarm",
				"name":      "Zero Value Hub Swarm",
				"maxAgents": 1,
			},
		},
		{
			...
			args: map[string]interface{}{
				"type": "coder",
				"task": "zero-value-hub-spawn",
			},
		},
		{
			name: "federation/terminate-ephemeral",
			args: map[string]interface{}{
				"agentId": "zero-value-agent",
			},
		},
		{
			name: "federation/broadcast",
			args: map[string]interface{}{
				"sourceSwarmId": "zero-value-source",
		...
		},
		{
			...
			args: map[string]interface{}{
				"voterId":    "zero-value-source",
				"proposalId": "zero-value-proposal",
				"approve":    true,
			},
		},
	}
...
func TestFederationTools_ExecuteAndExecuteTool_RegisterSwarmDuplicateIDParity(t *testing.T) {
	hub := federation.NewFederationHubWithDefaults()
	if err := hub.Initialize(); err != nil {
		t.Fatalf("failed to initialize federation hub: %v", err)
	}
	...
	args := map[string]interface{}{
		"swarmId":   "swarm-dup",
		"name":      "Duplicate Swarm",
		"maxAgents": float64(5),
	}

	execResult, execErr := ft.Execute(context.Background(), "federation/register-swarm", args)
	if execErr == nil {
		t.Fatal("expected Execute duplicate swarm registration to fail")
	}
	if execResult == nil {
		t.Fatal("expected Execute result")
	}
	...
	if directErr == nil {
		t.Fatal("expected ExecuteTool duplicate swarm registration to fail")
	}
...
func TestFederationTools_ExecuteAndExecuteTool_MutatingToolsRejectBeforeInitializeParity(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			toolName: "federation/propose",
			args: map[string]interface{}{
				"proposerId":   "preinit-proposer",
				"proposalType": "preinit-proposal",
				"value":        map[string]interface{}{"ok": true},
			},
		},
		{
			...
			toolName: "federation/vote",
			args: map[string]interface{}{
				"voterId":    "preinit-voter",
				"proposalId": "preinit-proposal-id",
				"approve":    true,
			},
		},
	}
...
func TestFederationTools_ExecuteAndExecuteTool_UnknownToolParityWithUninitializedHub(t *testing.T) {
	hub := federation.NewFederationHubWithDefaults()
	ft := NewFederationTools(hub)
	args := map[string]interface{}{}
...
func TestFederationTools_ExecuteAndExecuteTool_UnknownToolParityWithShutdownHub(t *testing.T) {
	hub := federation.NewFederationHubWithDefaults()
	if err := hub.Initialize(); err != nil {
		t.Fatalf("failed to initialize federation hub: %v", err)
	}
...
func TestFederationTools_ExecuteAndExecuteTool_UnknownToolParityWithZeroValueInternalHub(t *testing.T) {
	ft := NewFederationTools(&federation.FederationHub{})
	args := map[string]interface{}{}
